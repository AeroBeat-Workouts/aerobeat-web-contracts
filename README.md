...
- `src/body-grid-contracts.js` owns calibrated anchors, optional eight-way shoulder-relative cell-entry direction evidence, calibration/readiness, tracking-safety states (including the 0.0.60 mid-run `anchorsFrozen` flag and the `degradedAnchors` per-anchor dim set), and locked production defaults. Direction omission is a measured cell entry with ambiguous motion, not synthetic direction evidence.
- `src/input-shapes.js` retains additive compatibility for current gameplay-facing input events.
- `src/session-contracts.js` and `src/gameplay-contracts.js` own session/countdown/lease snapshots, explicit normal-Play versus unranked Visual-Test purpose, Flow/Boxing ruleset IDs (the sole Flow ruleset is exact `flow_colliders_v1`; the retired Flow Grid `flow_grid_v2` ID is deleted, with historical `flow_grid_v1` retained for legacy judgement reads), gameplay evidence (measured frames plus the 0.0.60 held "frozen" frame carrying the last measured frame's identity/timestamp and a per-tick `frozenTickId`), judgement diagnostics, exact judgement commitment timeline truth, recipes and prototype tuning identities. The strict Flow Colliders public summary contains only stable `mode:"flow"`/ruleset identity, a latest semantic note judgement, and bounded aggregate note/bomb/wall counts; it rejects private collider, pose, geometry, timing, and provenance evidence. Visual Test cannot enter calibration/tracking states; explicit real-collision Visual Test sessions may produce version-2 gameplay judgement truth while synthetic demonstration feedback remains renderer/assembly-local. Converter identities carry dynamic selected-vs-applied truth (`regenerationRequired` true while pending, false after matching generated-package provenance); live visual and between-run scoring identities always carry false.
- `src/obstacle-contracts.js` owns mode-neutral provider-evidence and canonical `aerobeat_top_left_grid` obstacle geometry, downward occupancy, and exact derived masks; source coordinate spaces are never render/collision authority.
...
- `src/equipment-pose-contracts.js` owns strict pure quaternion math, canonical Flow/Boxing composition, fixed-endpoint shortest-path pose resolution, the exact private renderer/gameplay resolved-pose record, transformed saber-capsule/glove-OBB geometry, and canonical equipment-config identity input. The identity authority accepts exact `aerobeat/equipment_config` versions 2, 3, and 4, preserves the established v2/v3 bytes, records the truthful config version, and rejects every other version. It has no browser, PlayCanvas, or engine dependency.
